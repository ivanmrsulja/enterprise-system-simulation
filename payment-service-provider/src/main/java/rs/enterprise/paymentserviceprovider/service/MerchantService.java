...
public class MerchantService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        return merchantRepository.findByMerchantId(username)
                .orElseThrow(() -> new NotFoundException(String.format("Merchant with ID: %s not found.", username)));
    }

    public Merchant register(RegisterDTO registrationRequest) {
        var apiKey = secureStringGenerator.generate(128);

        var authority = authorityRepository.findByName("ROLE_MERCHANT")
                .orElseThrow(() -> new NotFoundException("Authority with given name does not exist."));
        var user = new Merchant(registrationRequest.getMerchantId(),
                        passwordEncoder.encode(registrationRequest.getMerchantPassword()),
                        apiKey,
                        registrationRequest.getName(),
                        registrationRequest.getEmail(), authority);
